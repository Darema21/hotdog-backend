class Api::V1::BreedSerializer < ActiveModel::Serializer
  attributes :name, :description, :friendliness_index, :maintenance_index, :activity_index, :trainability_index,
             :average_life_expectancy, :average_weight, :average_height, :best_matches, :images_attachments
  ...
  def calculate_similarity_score(other_breed)
    ...
    friendliness_diff =  ((object.good_with_children + object.good_with_other_dogs + object.good_with_strangers) / 15.0 - (other_breed["good_with_children"] + other_breed["good_with_children"] + other_breed["good_with_children"]) / 15.0).abs / 1.0
