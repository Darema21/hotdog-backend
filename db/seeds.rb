...
#   { name: 'Dalmatian', image: 'app/assets/images/breeds/dalmatin.png', description: 'The Dalmatian is a distinctive and easily recognizable dog breed known for its white coat with black spots. Dalmatians have a playful and outgoing personality, making them great family pets and companions. They are often associated with firefighters and firehouses due to their historical role as carriage dogs and firehouse mascots. Dalmatians are energetic and require regular exercise and mental stimulation.' },
...
owners.each do |owner|
  ...
  1.times do
    ...
    # Attach three dog photos using the first three URLs in the shuffled array
    3.times do
      url = dog_image_urls.pop
      photo_file = URI.open(url)
      dog.images.attach(io: photo_file, filename: "#{dog.name.parameterize}_dog_#{dog.images.length + 1}.jpg", content_type: 'image/jpeg')
    end
...
10.times do
  ...
  if event.save
    ...
    if booking.save
      ...
      image_url = event_image_urls.pop
      event_image = URI.open(image_url)
      event.image.attach(io: event_image, filename: "#{event.title.parameterize}_event_image.jpg", content_type: 'image/jpeg')
